...
// ClientsSection animation refs interface
...
export function animateClientsSection(refs: ClientsAnimationRefs): void {
  ...
  const { section, title, text, logos, cta, decor } = refs;
  
  // Remove ScrollTrigger completely
  if (decor) {
    // Simple floating animation without ScrollTrigger
  ...
  // Simple fade-in animations without scroll trigger
  if (title) {
    gsap.fromTo(
      title, 
      { opacity: 0, y: 50 }, 
  ...
  if (logos) {
    gsap.fromTo(
      logos.querySelectorAll('.clients-section__logo'),
      {
        opacity: 0,
        y: 30
      },
